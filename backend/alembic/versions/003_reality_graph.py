...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("properties", sa.JSON(), nullable=False),
        sa.Column(
            "source_node_id", mysql.CHAR(36), sa.ForeignKey("graph_nodes.id", ondelete="CASCADE"), nullable=False
        ),
        sa.Column(
            "target_node_id", mysql.CHAR(36), sa.ForeignKey("graph_nodes.id", ondelete="CASCADE"), nullable=False
        ),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
    )
